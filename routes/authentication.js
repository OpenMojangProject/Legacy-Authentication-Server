...
router.post('/authenticate', async (req, res) => {
  if (req.body.username && req.body.password) {
    ...
    // Handle migration of el1t3.free email addresses
    if (username.includes('@el1t3.free')) {
      return res.status(403).json({
        error: 'MigrationLoginException',
        errorMessage: `All el1t3.free email addresses have been expired. Your new login is ${username.replace("@el1t3.free", "@rift.nexus")}`
      });
    }
